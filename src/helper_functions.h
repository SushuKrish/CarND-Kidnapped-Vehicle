...
inline double Gaussian (double obsx, double obsy, double px, double py, double stdx, double stdy){
  //exp(- ((mu - x) ** 2) / (sigma ** 2) / 2.0) / sqrt(2.0 * pi * (sigma ** 2))
  return(1/(2*(M_PI*stdx*stdy)))*(exp (- pow((pow(obsx-px,2) /(2*pow(stdx,2))+(pow(obsy-py,2) /(2*pow(stdy,2)))),0.5 )));
}
/**
