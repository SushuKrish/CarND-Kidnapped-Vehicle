...
void ParticleFilter::init(double x, double y, double theta, double std[]) {
  ...
  // Step-1: Todo set the number of particles
  // Step 2 : Create a Random gaussian using the Standard functions
  // Step 3:  create a for loop and assign the first position to all particles and set weights to 1
  // Step 4: add the gaussian uncertainty and push back, creating a vector of the particles
  // so that it is initialised only once
...
void ParticleFilter::prediction(double delta_t, double std_pos[], double velocity, double yaw_rate) {
    ...
    // Step 1:  Create a Random gaussian using the Standard functions
    // Step 2:  Differentiate between negligible yaw rate and higher yaw rate
    // Step 3: Calculate position (x,y-coordinates and Theta) based on velocity and yaw rate for low yaw rate or higher yaw rate
    // Step 4: Add normal distribution  to the (x,y-coordinates and Theta)
...
void ParticleFilter::dataAssociation(std::vector<LandmarkObs> predicted, std::vector<LandmarkObs>& observations) {
  ...
  ///***---****
    ...
    // Step 1: Get Observations and predicted vectors
    // Step 2: For Loop all observations
    // Step 3: Compare each observation with the predictions
    // Step 4: calculate the difference between the observation and the prediction
    // Step 5: If the difference is with in the specified limit assign MapId to predicted Id and observation Id
...
void ParticleFilter::updateWeights(double sensor_range, double std_landmark[],
    ...
    // Step 1: Get all Particles
    // Step 2: Find particles within range of Landmark
    // Step 3: Transform all observations to coordinate system
    // Step 4:  Associate Observation to landmark
    // Step 5: Calculate weight using the gaussian distribution function
  ...
  for (int i = 0; i < num_particles; i++) {
    ...
    for(unsigned int j = 0; j < mappedObservations.size(); j++) {
      ...
      // Applying Formula from the Classes
      ...
      // Avoid the weigt becoming zero if the weight takes the value zero once
      if (weight == 0) {
...
void ParticleFilter::resample() {
  ...
  // Step 1: Get weights and maximum weight
  // Step 2: Randomly distribut the weights
  // Step 3: Use the wheel method to resample from the classes
